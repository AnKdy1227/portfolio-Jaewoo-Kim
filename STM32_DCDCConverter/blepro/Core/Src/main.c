...
uint32_t pwm_freq_hz = 133000; // 기본 주파수
uint32_t duty_cycle_percent = 40;
uint32_t deadtime_tick = 0;
    ...
    if (huart->Instance == USART1)
    {
        ...
        if (c == '\r' || c == '\n') {
            ...
            if (strcmp(UART_Buffer, "start") == 0) {
               ...
               /* softstart
                PWM_SoftStart(duty_cycle_percent, 100);  // 1% per 10ms
                */
...
/* USER CODE END 0 */
// DEBUGING CODE whether your keyboard input works
//void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
//{
//    if (huart->Instance == USART2)
//    {
//        char echo[] = "callback hit\r\n";
...
char msg[] ="=== PWM Control Interface ===\r\n"
	      "Current PWM: OFF\r\n"
		  "Type '*<number>' to set frequency before starting"
	      "Type 'start' to begin PWM, 'stop' to stop it.\r\n"
	      "Type '*100000' to set frequency to 100kHz\r\n"
	      "==============================\r\n";
...
/* softstart function def
void PWM_SoftStart(uint32_t target_duty, uint32_t step_delay_ms)
{
    uint32_t timer_clk = 72000000;  // 72 MHz
    uint32_t period = timer_clk / pwm_freq_hz;

    __HAL_TIM_SET_COUNTER(&htim1, 0);
    HAL_TIM_PWM_Start(&htim1, TIM_CHANNEL_1);
    HAL_TIMEx_PWMN_Start(&htim1, TIM_CHANNEL_1);

    for (uint32_t duty = 0; duty <= target_duty; duty++)
    {
        uint32_t pulse = (period * duty) / 100;
        TIM1->CCR1 = pulse;  // 실시간 듀티 변경

        HAL_Delay(step_delay_ms);
    }

    duty_cycle_percent = target_duty;
}
*/
